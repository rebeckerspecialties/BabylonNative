...
namespace {
    ...
    /**
     Defines the 2D positions and app texture UVs. Camera UVs are generated
     from cameraViewportUVs below using ARKit's display transform; do not reuse
     the app texture UVs for that path or camera passthrough will inherit app
     texture orientation fixes.
     NOTE: The WebGPU-backed XR app texture arrives in the orientation expected
     for final display, so the app UVs are kept in screen order. The ARKit
     camera path uses cameraViewportUVs below and remains independent.
     */
    ...
    static const CGPoint cameraViewportUVs[] = {
        { 0, 1 },
        { 0, 0 },
        { 1, 1 },
        { 1, 0 },
    ...
    float ResolveNativeXrRenderScale()
    {
        static const float renderScale = []() {
            const char* value = std::getenv("BABYLON_NATIVE_XR_RENDER_SCALE");
            ...
            errno = 0;
            char* end = nullptr;
            const auto parsedValue = std::strtof(value, &end);
            if (end == value || errno == ERANGE || !std::isfinite(parsedValue)) {
                return 1.0f;
            }

            return std::max(0.25f, std::min(parsedValue, 1.0f));
        }();

        return renderScale;
    }

    uint32_t ScaleDrawableDimension(uint32_t dimension, float scale)
    {
        return std::max<uint32_t>(1, static_cast<uint32_t>(std::lround(static_cast<float>(dimension) * scale)));
    }
    ...
    static uint32_t ResolveDrawableDimension(CGFloat primary, CGFloat secondary, CGFloat tertiary, CGFloat fallback) {
        auto choose = [](CGFloat value) -> uint32_t {
            constexpr auto MAX_METAL_DRAWABLE_DIMENSION = 16384.0;
            if (!std::isfinite(static_cast<double>(value)) || value <= 0 || value > MAX_METAL_DRAWABLE_DIMENSION) {
                return 0;
            }

            return static_cast<uint32_t>(std::lround(static_cast<double>(value)));
        };

        if (auto value = choose(primary); value > 0) {
            return value;
        }
        if (auto value = choose(secondary); value > 0) {
            return value;
        }
        if (auto value = choose(tertiary); value > 0) {
            return value;
        }
        if (auto value = choose(fallback); value > 0) {
            return value;
        }
    ...
    static uint64_t ResolveCaptureFrame()
    {
        const char* value = std::getenv("BABYLON_NATIVE_XR_CAPTURE_FRAME");
        if (value == nullptr || *value == '\0')
        {
            return 0;
        }
        ...
        errno = 0;
        char* end{};
        const auto frame = std::strtoull(value, &end, 10);
        if (errno != 0 || end == value || frame == 0)
        {
            return 0;
        }
    ...
    static NSString* ResolveCapturePath(uint64_t frameIndex)
    {
        if (const char* explicitPath = std::getenv("BABYLON_NATIVE_XR_CAPTURE_PATH"))
        {
            if (*explicitPath != '\0')
            {
                return [NSString stringWithUTF8String:explicitPath];
            }
        }
        ...
        NSString* resultsPath = [outputPath stringByAppendingPathComponent:@"Results"];

        NSString* filename{};
        if (const char* name = std::getenv("BABYLON_NATIVE_XR_CAPTURE_NAME"))
        {
            filename = *name != '\0' ? [NSString stringWithUTF8String:name] : nil;
        }
        if (filename == nil)
        {
            filename = [NSString stringWithFormat:@"native_xr_wgpu_composite_%llu.png", frameIndex];
        }

        return [resultsPath stringByAppendingPathComponent:filename];
    }

    static bool WriteRGBA8Png(NSString* path, const uint8_t* data, uint32_t width, uint32_t height)
    {
        if (path == nil || data == nullptr || width == 0 || height == 0)
        {
            return false;
        }
        ...
        {
            NSLog(@"[NativeXR] failed to create capture directory %@: %@", directory, directoryError);
            return false;
        }
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(nullptr, data, static_cast<size_t>(width) * static_cast<size_t>(height) * 4u, nullptr);
        if (provider == nullptr)
        {
    ...
    static void CaptureDrawableTexture(id<MTLDevice> device, id<MTLCommandBuffer> commandBuffer, id<MTLTexture> texture, uint64_t frameIndex)
    {
        if (device == nil || commandBuffer == nil || texture == nil || texture.width == 0 || texture.height == 0)
        {
            return;
        }
        ...
        const NSUInteger byteLength = bytesPerRow * static_cast<NSUInteger>(height);
        ...
        {
            NSLog(@"[NativeXR] failed to allocate capture readback buffer for %ux%u", width, height);
            return;
        }
        ...
        [commandBuffer addCompletedHandler:^(id<MTLCommandBuffer> completedCommandBuffer) {
            ...
            {
                NSLog(@"[NativeXR] capture command buffer did not complete: status=%lu error=%@", static_cast<unsigned long>(completedCommandBuffer.status), completedCommandBuffer.error);
                return;
            }

            const uint8_t* bgra = static_cast<const uint8_t*>(readbackBuffer.contents);
            if (bgra == nullptr)
            {
                return;
            }
            ...
            NSString* path = ResolveCapturePath(frameIndex);
            if (WriteRGBA8Png(path, rgba.data(), width, height))
            {
                NSLog(@"[NativeXR] wrote XR composite capture %@ (%ux%u)", path, width, height);
            }
            else
            {
                NSLog(@"[NativeXR] failed to write XR composite capture %@", path);
            }
        }];
    }
}
...
- (void)session:(ARSession *)__unused session didUpdateFrameInternal:(ARFrame *)frame {
    ...
    // Finally update the XR pose based on the current camera-to-world transform from ARKit.
    [self updatePose:(frame.camera)];
}
...
- (Boolean)checkAndUpdateCameraUVs:(ARFrame *)frame {
    ...
    if (cameraUVReferenceOrientation != orientation || cameraUVReferenceSize.height != viewportSize.height || cameraUVReferenceSize.width != viewportSize.width) {
        ...
        for(size_t i = 0; i < sizeof(vertices) / sizeof(*vertices); i++) {
            CGPoint transformedPoint = CGPointApplyAffineTransform(cameraViewportUVs[i], transform);
...
/**
 Updates the WebXR view pose from ARKit's camera-to-world transform.

 Projection and camera UVs are display-oriented separately. The WebXR pose
 should remain in the XR reference space; rotating it into interface
 orientation double-applies display orientation and makes world-locked content
 drift relative to the camera feed in portrait.
 */
-(void)updatePose:(ARCamera*)camera {
    auto& frameView = activeFrameViews->at(0);
    simd_float4x4 transform = [camera transform];
    auto orientation = simd_quaternion(transform);

    // Set the orientation of the camera
    frameView.Space.Pose.Orientation = { orientation.vector.x
        , orientation.vector.y
        , orientation.vector.z
        , orientation.vector.w};
...
- (void)mtkView:(MTKView *)__unused view drawableSizeWillChange:(CGSize)size {
    auto scale = view.contentScaleFactor;
    auto drawableSize = view.drawableSize;
    auto screenBounds = UIScreen.mainScreen.bounds.size;
    _viewportSize.width = ResolveDrawableDimension(size.width, drawableSize.width, _viewportSize.width, screenBounds.width * scale);
    _viewportSize.height = ResolveDrawableDimension(size.height, drawableSize.height, _viewportSize.height, screenBounds.height * scale);
}
...
- (CGSize)viewSize {
    return CGSizeMake(
        ResolveDrawableDimension(_viewportSize.width, 0, 0, 1),
        ResolveDrawableDimension(_viewportSize.height, 0, 0, 1));
}

- (void)setViewSize:(CGSize)size {
    _viewportSize.width = ResolveDrawableDimension(size.width, _viewportSize.width, 0, 1);
    _viewportSize.height = ResolveDrawableDimension(size.height, _viewportSize.height, 0, 1);
}
...
namespace xr {
    namespace {
        ...
        constexpr char shaderSource[] = R"(
                ...
                texture2d<float, access::sample> cameraTextureCbCr [[ texture(2) ]])
            {
                ...
                float4 cameraSample = float4(0.0, 0.0, 0.0, 1.0);
                const bool hasCamera = !is_null_texture(cameraTextureY) && !is_null_texture(cameraTextureCbCr);
                if (hasCamera)
                {
                    ...
                    cameraSample = ycbcrToRGBTransform * ycbcr;
                    cameraSample.a = 1.0;
                }

                if (!is_null_texture(babylonTexture))
                {
                    const float4 appSample = babylonTexture.sample(linearSampler, in.uv);
                    if (hasCamera)
                    {
                        return float4(mix(cameraSample.rgb, appSample.rgb, appSample.a), 1.0);
                    }
            ...
            fragment float4 solidColorFragmentShader(RasterizerData in [[stage_in]])
            {
    ...
    public:
        ...
        Impl(System::Impl& systemImpl, void* graphicsContext, void* commandQueue, std::function<void*()> windowProvider)
            ...
            , commandQueue{ commandQueue != nullptr ? (__bridge id<MTLCommandQueue>)commandQueue : [metalDevice newCommandQueue] } {

            if (metalDevice == nil || this->commandQueue == nil) {
                throw std::runtime_error{"ARKit XR requires a valid Metal device and command queue."};
            }

            if (traceEnabled || cameraOnlyMode || debugSolidColorMode) {
                NSLog(@"[NativeXR] ARKit session init trace=%d cameraOnly=%d debugSolid=%d", traceEnabled ? 1 : 0, cameraOnlyMode ? 1 : 0, debugSolidColorMode ? 1 : 0);
            }
            ...
            id<MTLFunction> solidColorFragmentFunction = [lib newFunctionWithName:@"solidColorFragmentShader"];
            ...
            // Diagnostic path: verifies the XR MTKView/CAMetalLayer is visible
            // on the device without relying on camera or app texture sampling.
            {
                MTLRenderPipelineDescriptor *pipelineStateDescriptor = [[MTLRenderPipelineDescriptor alloc] init];
                pipelineStateDescriptor.label = @"XR Debug Solid Color Pipeline";
                pipelineStateDescriptor.vertexFunction = vertexFunction;
                pipelineStateDescriptor.fragmentFunction = solidColorFragmentFunction;
                pipelineStateDescriptor.colorAttachments[0].pixelFormat = MTLPixelFormatBGRA8Unorm;

                NSError* error;
                solidColorPipelineState = [metalDevice newRenderPipelineStateWithDescriptor:pipelineStateDescriptor error:&error];
                if (!solidColorPipelineState) {
                    NSLog(@"Failed to create solid color pipeline state: %@", error);
                }
            }
        }
        ...
        void UpdateXRView(MTKView* activeXRView) {
            ...
            if (activeXRView != xrView) {
                ...
                if (xrView) {
                    ...
                    if (captureFrameIndex != 0) {
                        xrView.framebufferOnly = false;
                    }
...
#pragma clang diagnostic pop
            ...
            if (xrView) {
                ...
                auto drawableSize = xrView.drawableSize;
                auto layerDrawableSize = metalLayer != nil ? metalLayer.drawableSize : CGSizeZero;
                auto screenBounds = UIScreen.mainScreen.bounds.size;
                viewportSize.x = ResolveDrawableDimension(drawableSize.width, layerDrawableSize.width, xrView.bounds.size.width * scale, screenBounds.width * scale);
                viewportSize.y = ResolveDrawableDimension(drawableSize.height, layerDrawableSize.height, xrView.bounds.size.height * scale, screenBounds.height * scale);
        ...
        std::unique_ptr<System::Session::Frame> GetNextFrame(bool& shouldEndSession, bool& shouldRestartSession, std::function<arcana::task<void, std::exception_ptr>(void*)> deletedTextureAsyncCallback) {
            ...
            viewportSize.x = ResolveDrawableDimension(viewSize.width, 0, 0, 1);
            viewportSize.y = ResolveDrawableDimension(viewSize.height, 0, 0, 1);
            const auto renderScale = ResolveNativeXrRenderScale();
            uint32_t width = ScaleDrawableDimension(viewportSize.x, renderScale);
            uint32_t height = ScaleDrawableDimension(viewportSize.y, renderScale);
            if (renderScale < 1.0f) {
                static bool didLogRenderScale = false;
                if (!didLogRenderScale) {
                    didLogRenderScale = true;
                    NSLog(@"[NativeXR] render scale %.3f app=%ux%u viewport=%ux%u",
            ...
            if (ActiveFrameViews[0].ColorTextureSize.Width != width || ActiveFrameViews[0].ColorTextureSize.Height != height) {
                ...
                // Allocate and store the depth texture
                {
                    ...
                    ActiveFrameViews[0].DepthTextureFormat = TextureFormat::D32FS8;
                    ActiveFrameViews[0].DepthTextureSize = {width, height};
                }
            }

            // Browser WebXR AR compositors render the app layer over camera
            // passthrough. Preserve that model here: Babylon clears/renders an
            // alpha layer, and DrawFrame composites it over the latest ARKit
            // camera image in the final native pass.
            ActiveFrameViews[0].RequiresAppClear = true;

            if (std::getenv("BABYLON_NATIVE_XR_PRECOMPOSE_CAMERA") != nullptr) {
        ...
        void DrawFrame() {
            if (metalLayer) {
                ...
                if (drawable == nil) {
                    if (traceEnabled && (drawFrameCount % 60 == 0)) {
                        NSLog(@"[NativeXR] draw skipped because CAMetalLayer returned no drawable viewport=%ux%u cameraOnly=%d",
                            viewportSize.x,
                            viewportSize.y,
                            cameraOnlyMode ? 1 : 0);
                    }
                ...
                if (renderPassDescriptor != nil) {
                    ...
                    [renderEncoder setRenderPipelineState:debugSolidColorMode ? solidColorPipelineState : screenPipelineState];

                    // Set the vertex data.
                    [renderEncoder setVertexBytes:vertices length:sizeof(vertices) atIndex:0];

                    id<MTLTexture> appTexture = (cameraOnlyMode || debugSolidColorMode) ? nil : (__bridge id<MTLTexture>)ActiveFrameViews[0].ColorTexturePointer;
                    const auto frameIndex = ++drawFrameCount;
                    if (traceEnabled && (frameIndex % 60 == 1)) {
                        auto cameraYWidth = cameraTextureY != nil ? cameraTextureY.width : 0;
                        auto cameraYHeight = cameraTextureY != nil ? cameraTextureY.height : 0;
                        auto cameraCbCrWidth = cameraTextureCbCr != nil ? cameraTextureCbCr.width : 0;
                        auto cameraCbCrHeight = cameraTextureCbCr != nil ? cameraTextureCbCr.height : 0;
                        auto appWidth = appTexture != nil ? appTexture.width : 0;
                        auto appHeight = appTexture != nil ? appTexture.height : 0;
                        NSLog(@"[NativeXR] draw cameraY=%dx%d cameraCbCr=%dx%d app=%dx%d viewport=%ux%u cameraOnly=%d debugSolid=%d",
                            static_cast<int>(cameraYWidth),
                            static_cast<int>(cameraYHeight),
                            static_cast<int>(cameraCbCrWidth),
                            static_cast<int>(cameraCbCrHeight),
                            static_cast<int>(appWidth),
                            static_cast<int>(appHeight),
                            viewportSize.x,
                            viewportSize.y,
                            cameraOnlyMode ? 1 : 0,
                            debugSolidColorMode ? 1 : 0);
                    }

                    // Set the textures.
                    if (!debugSolidColorMode) {
                        [renderEncoder setFragmentTexture:appTexture atIndex:0];
                        [renderEncoder setFragmentTexture:cameraTextureY atIndex:1];
                        [renderEncoder setFragmentTexture:cameraTextureCbCr atIndex:2];
                    }
                    ...
                    if (captureFrameIndex != 0 && !captureWritten && frameIndex >= captureFrameIndex) {
                        captureWritten = true;
                        CaptureDrawableTexture(metalDevice, currentCommandBuffer, drawable.texture, frameIndex);
                    }

                    [currentCommandBuffer addCompletedHandler:^(id<MTLCommandBuffer>) {
                        (void)cameraTextureY;
                        (void)cameraTextureCbCr;
                    }];
         ...
         */
        void UpdatePlanes(
            std::vector<Frame::SceneObject::Identifier>& updatedSceneObjects,
            std::vector<Frame::SceneObject::Identifier>& deletedSceneObjects,
            std::vector<Frame::Plane::Identifier>& updatedPlanes,
            ...
            @try {
                ...
                for (ARPlaneAnchor* updatedPlane : *updatedARKitPlanes) {
                    ...
                    const std::string planeKey{[updatedPlane.identifier.UUIDString UTF8String]};
                    auto planeIterator = planeMap.find(planeKey);
                    if (planeIterator != planeMap.end()) {
                        auto& plane = GetPlaneByID(planeIterator->second);
                        UpdatePlane(updatedPlanes, plane, updatedPlane, planePolygonBuffer, polygonSize);
                        UpdatePlaneSceneObject(updatedSceneObjects, plane, updatedPlane, planeKey);
                    } else {
                        // This is a new plane, create it and initialize its values.
                        Planes.emplace_back();
                        auto& plane = Planes.back();
                        planeMap.insert({planeKey, plane.ID});
                ...
                for (ARPlaneAnchor* removedPlane: *removedARKitPlanes) {
                    ...
                    const std::string planeKey{[removedPlane.identifier.UUIDString UTF8String]};
                    auto planeIterator = planeMap.find(planeKey);
                    if (planeIterator != planeMap.end()) {
                        ...
                        const auto sceneObjectIterator = planeSceneObjectMap.find(nativePlaneID);
                        if (sceneObjectIterator != planeSceneObjectMap.end()) {
                            deletedSceneObjects.push_back(sceneObjectIterator->second);
                            auto& sceneObject = GetSceneObjectByID(sceneObjectIterator->second);
                            sceneObject.Type = SceneObjectType::Undefined;
                            planeSceneObjectMap.erase(sceneObjectIterator);
                        }
                        plane.ParentSceneObjectID = Frame::SceneObject::INVALID_ID;
                        plane.Polygon.clear();
                        plane.PolygonSize = 0;
        ...
        Frame::SceneObject& GetSceneObjectByID(Frame::SceneObject::Identifier sceneObjectID)
        {
            const auto end{SceneObjects.end()};
            const auto it{std::find_if(
                SceneObjects.begin(),
                end,
                [&](Frame::SceneObject& sceneObject) { return sceneObject.ID == sceneObjectID; })};
            if (it != end) {
                return *it;
            } else {
                throw std::runtime_error{"Tried to get non-existent scene object."};
            }
        }
    ...
    private:
        ...
        bool traceEnabled{ std::getenv("BABYLON_NATIVE_XR_TRACE") != nullptr };
        bool cameraOnlyMode{ std::getenv("BABYLON_NATIVE_XR_CAMERA_ONLY") != nullptr };
        bool debugSolidColorMode{ std::getenv("BABYLON_NATIVE_XR_DEBUG_SOLID_COLOR") != nullptr };
        uint64_t captureFrameIndex{ ResolveCaptureFrame() };
...
#pragma clang diagnostic pop
        ...
        std::unordered_map<std::string, Frame::SceneObject::Identifier> planeSceneObjectMap{};
        ...
        SceneObjectType GetSceneObjectTypeForPlane(ARPlaneAnchor* planeAnchor) const {
            if (@available(iOS 12.0, *)) {
                if ([ARPlaneAnchor isClassificationSupported]) {
                    switch (planeAnchor.classificationStatus) {
                        ...
                        default:
                            return SceneObjectType::Inferred;
                    }
                    ...
                    switch (planeAnchor.classification) {
                        ...
                        case ARPlaneClassificationDoor:
                            return SceneObjectType::Wall;
                        case ARPlaneClassificationFloor:
                            return SceneObjectType::Floor;
                        case ARPlaneClassificationCeiling:
                            return SceneObjectType::Ceiling;
        ...
        void UpdatePlaneSceneObject(std::vector<Frame::SceneObject::Identifier>& updatedSceneObjects, Frame::Plane& plane, ARPlaneAnchor* planeAnchor, const std::string& planeKey) {
            auto sceneObjectIterator = planeSceneObjectMap.find(planeKey);
            bool sceneObjectCreated = false;
            if (sceneObjectIterator == planeSceneObjectMap.end()) {
                SceneObjects.emplace_back();
                auto& sceneObject = SceneObjects.back();
                sceneObjectIterator = planeSceneObjectMap.insert({planeKey, sceneObject.ID}).first;
                sceneObjectCreated = true;
            }

            auto& sceneObject = GetSceneObjectByID(sceneObjectIterator->second);
            const auto type = GetSceneObjectTypeForPlane(planeAnchor);
            plane.ParentSceneObjectID = sceneObject.ID;
            if (sceneObjectCreated || sceneObject.Type != type) {
                sceneObject.Type = type;
                updatedSceneObjects.push_back(sceneObject.ID);
            }
        }
        ...
        void GetHitTestResultsForiOS13(std::vector<HitResult>& filteredResults, xr::Ray offsetRay, xr::HitTestTrackableType trackableTypes) const{
            ...
            auto rayOrigin = cameraOrigin + simd_act(cameraDirection, offsetOrigin);
            ...
            if ((trackableTypes & xr::HitTestTrackableType::PLANE) != xr::HitTestTrackableType::NONE) {
                PerformRaycastQueryAgainstTarget(filteredResults, ARRaycastTargetExistingPlaneGeometry, rayOrigin, rayDirection, ARRaycastTargetAlignmentHorizontal);
                PerformRaycastQueryAgainstTarget(filteredResults, ARRaycastTargetExistingPlaneInfinite, rayOrigin, rayDirection, ARRaycastTargetAlignmentHorizontal);
            }

            if ((trackableTypes & xr::HitTestTrackableType::POINT) != xr::HitTestTrackableType::NONE) {
                PerformRaycastQueryAgainstTarget(filteredResults, ARRaycastTargetEstimatedPlane, rayOrigin, rayDirection, ARRaycastTargetAlignmentHorizontal);
            }
        ...
        API_AVAILABLE(ios(13.0))
        void PerformRaycastQueryAgainstTarget(
            std::vector<HitResult>& filteredResults,
            ARRaycastTarget targetType,
            simd_float3 origin,
            simd_float3 direction,
            ARRaycastTargetAlignment alignment = ARRaycastTargetAlignmentAny) const {
    ...
    System::Session::Frame::Frame(Session::Impl& sessionImpl)
        ...
        m_impl->sessionImpl.UpdatePlanes(UpdatedSceneObjects, RemovedSceneObjects, UpdatedPlanes, RemovedPlanes);
#if (__IPHONE_OS_VERSION_MAX_ALLOWED >= 130400)
    ...
    System::Session::Frame::SceneObject& System::Session::Frame::GetSceneObjectByID(System::Session::Frame::SceneObject::Identifier sceneObjectID) const {
        return m_impl->sessionImpl.GetSceneObjectByID(sceneObjectID);
    }
